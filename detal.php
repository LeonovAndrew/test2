<?php include_once __DIR__."/header.php";?>
<section>
    <div class="container">
        <div class="row">
            <div class="col-sm-6">
                <h2>
                    Кальян Alpha Hookah X Black
                    atte (Черный мат)
                </h2>
            </div>
            <div class="col-sm-6">
            </div>
        </div>
        <div class="row">
            <div class="col-sm-6">
                <div class="detal_product_slider_wrap">
                    <div class="col_slider_for">
                        <div class="slider-for">
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr1.png" data-lightbox="product">
                                    <img src="image/prod/pr1.png" alt="prod">
                                </a>
                            </div>
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr2.png" data-lightbox="product">
                                    <img src="image/prod/pr2.png" alt="prod">
                                </a>
                            </div>
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr3.png" data-lightbox="product">
                                    <img src="image/prod/pr3.png" alt="prod">
                                </a>
                            </div>
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr4.png" data-lightbox="product">
                                    <img src="image/prod/pr4.png" alt="prod">
                                </a>
                            </div>
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr5.png" data-lightbox="product">
                                    <img src="image/prod/pr5.png" alt="prod">
                                </a>
                            </div>
                            <div class="slider_for_wrap">
                                <a href="image/prod/pr6.png" data-lightbox="product">
                                    <img src="image/prod/pr6.png" alt="prod">
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col_slider_nav">
                        <div class="slider-nav">
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr1.png" alt="prod">
                            </div>
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr2.png" alt="prod">
                            </div>
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr3.png" alt="prod">
                            </div>
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr4.png" alt="prod">
                            </div>
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr5.png" alt="prod">
                            </div>
                            <div class="slider_nav_wrap">
                                <img src="image/prod/pr6.png" alt="prod">
                            </div>
                        </div>
                    </div>

                </div>
            </div>
            <div class="col-sm-6">
                <div class="bonus_wrap">
                    +300 бонусов на карту
                </div>
                <div class="price_wrap_det">
                    <h2 class="price_det">
                        2500 р
                    </h2>
                </div>
                <div class="descript">
                    <p>
                        Лаконичность главная изюминка модели Mini-M, позволяющий взять свой любимый кальян в поездку, а может и освободить немного пространства дома. MattPear зарекомендовавшая себя компания на рынке, которая не требует лишних слов.
                    </p>
                </div>
                <div class="det_btn_wrap">
                    <div class="row">
                        <div class="col-sm-4 col-6 step_input_wrap">
                            <div class="stepper stepper--style-2 js-spinner">
                                <input autofocus="" type="number" min="1" max="100" step="1" value="1" class="stepper__input" data-stepper-debounce="400">
                                <div class="stepper__controls">
                                    <button type="button" spinner-button="up">+</button>
                                    <button type="button" spinner-button="down">-</button>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-4 col-6 det_btn_cart_wrap">
                            <button class="btn_main">В корзину</button>
                        </div>
                        <div class="col-sm-4 col-12 btn_fast_det_wrap">
                            <button class="btn_main light"  data-toggle="modal" data-target="#fast_click">
                                    <span class="text_for_line">
                                        В 1 клик
                                    </span>
                                <span  class="text_for_gird">
                                        Купить в 1 клик
                                    </span>
                            </button>
                        </div>
                    </div>

                </div>
                <div class="det_icon_wrap">
                    <ul class="det_icon">
                        <li>
                            <a href="#">
                                <img src="image/svg/sr.svg" alt="Сравнение">
                                Добавить в<br> сравнение
                            </a>

                        </li>
                        <li>

                        </li>
                        <li>
                            <img src="image/svg/red_heart.svg" alt="Избранное">
                        </li>
                    </ul>
                </div>
                <h4 class="count_in_shop">
                    <span class="main_color">
                        в наличии:
                    </span>
                    <span class="text_underline" data-toggle="modal" data-target="#count_shop">
                        в 3 магазинах
                    </span>
                </h4>
                <div class="det_cart_inf">
                    <p>
                        Доступно в кредит и рассрочку в розничных магазинах
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-7 col-md-12">
                <h2>
                    Комплектация кальяна:
                </h2>
                <div class="row">
                    <div class="col-sm-5">
                        <img src="image/svg/kalian.svg" alt="kalian">
                    </div>
                    <div class="col-sm-7">
                        <table class="complect_table">
                            <tr data-toggle="tooltip" title="Клауд для кальяна достаточно Фольга при использовании клауда - не нужна.">
                                <td class="count_td">
                                    <span>1</span>
                                </td>
                                <td class="name_complect">
                                    <span>Калауд</span>
                                </td>
                                <td class="img_complect check">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                </td>
                            </tr>
                            <tr data-toggle="tooltip" title="Клауд для кальяна достаточно удобное в использовании приспособление для.">
                                <td class="count_td">
                                    <span>2</span>
                                </td>
                                <td class="name_complect">
                                    <span>Чаша</span>
                                </td>
                                <td class="img_complect check">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>3</span>
                                </td>
                                <td class="name_complect">
                                    <span>Блюдце</span>
                                </td>
                                <td class="img_complect check">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>4</span>
                                </td>
                                <td class="name_complect">
                                    <span>Шахта</span>
                                </td>
                                <td class="img_complect check">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>5</span>
                                </td>
                                <td class="name_complect">
                                    <span>Силиконовый шланг</span>
                                </td>
                                <td class="img_complect check">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>6</span>
                                </td>
                                <td class="name_complect">
                                    <span>Мундштук</span>
                                </td>
                                <td class="img_complect">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                    <a href="#">Докупить</a>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>7</span>
                                </td>
                                <td class="name_complect">
                                    <span>Колба</span>
                                </td>
                                <td class="img_complect">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                    <a href="#">Докупить</a>
                                </td>
                            </tr>
                            <tr>
                                <td class="count_td">
                                    <span>8</span>
                                </td>
                                <td class="name_complect">
                                    <span>Диффузор</span>
                                </td>
                                <td class="img_complect">
                                    <span class="check_img">
                                        <img src="image/svg/checkbig.svg" alt="check">
                                    </span>
                                    <span class="no_check_img">
                                        <img src="image/svg/no_check.svg" alt="no_check">
                                    </span>
                                    <a href="#">Докупить</a>
                                </td>
                            </tr>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="sect_wrap">
    <div class="container">
        <div class="dropdown">
            <button type="button" class=" tabs_btn btn dropdown-toggle" data-toggle="dropdown">
                Описание
            </button>
            <div class="dropdown-menu mob_tabs">
                <a class="dropdown-item" href="#opisanie">Описание</a>
                <a class="dropdown-item" href="#haracterist">Характеристики</a>
                <a class="dropdown-item" href="#comment">Отзывы</a>
            </div>
        </div>
        <!-- Nav tabs -->
        <ul class="nav nav-tabs tabs_desct" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" data-toggle="tab" href="#opisanie">Описание</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#haracterist">Характеристики</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#comment">Отзывы</a>
            </li>
        </ul>

        <!-- Tab panes -->
        <div class="tab-content">
            <div id="opisanie" class="container tab-pane active"><br>
                <div class="tabs_wrap">
                    <p>Компания Custom Water Pipes (CWP) набирает свои обороты с 2009 года и это уже делеко не маленький срок. По 2014 год компания выпускала только стеклянные дизайнерские кальяны такие как Gravity, Original, Shifter, а в конце 2014 года ребята задумались о производстве не только стекла , но и  шахты. Этот шаг оказался верным, потому что стекло останется стеклом, а металлическая шахта - это классика.  Так на свете появился кальян CWP Razor. Как и многие современные продукты рынка, кальян производится в Китае, но это никак не влияет на качества этого кальяна.

                    <p>Немного скучной ерунды для тех кто любит цифры:<br>
                    Высота кальяна целых 650 мм, не маленький , в обычный рюкзак не влезет. Соединение с колбой - 38 мм, влетит почти в любую колбу, будь это хоть богемка, канделуп или обычная мамуновская колба. Вес шахты – 1,5 кг, нормальный вес для кальяна, можно возить в багажнике как дубинку.

                    <p>О цене и комплектации<br>
                    Купить кальян CWP Razor вы можете начиная от 17500 наших деревянных на сайте производителей. В комплекте будет стильная коробка, которую жалко будет выбрасывать, шахта, блюдце с сеточкой, продувочный клапан, коннектор для шланга, фирменный уплотнитель для чаши, уплотнитель для шланга, а так же два уплотнителя для колбы.

                    <p>На первый взгляд кажется, что цена на кальян CWP Razor завышена, если мы все таки хотим на нем сразу покурить, то нам придется докупать кое-что отдельно, а именно колбу, шланг и чашку.  Но производители об этом позаботились и предлагают для полной комплектации рекомендованного производителя - свою дочернюю фирму 50 clouds. Среди ассортимента легко выбрать и купить колбу, а так же чашки, шланги, мундштуки. Цены не очень дешевые, но товар стильный и качественный. Чтобы не пришлось все искать в разных местах, у нас вы сможете найти и купить в онлайн режиме не только сам кальян CWP Razor, но и все необходимые аксуссуары прямо с доставкой по вашему адресу.

                    <p>У кальяна крутой дизайн, хотя многие путают его с продуктос ОтИвана. Доступен в 2-х цветах: черном и бронзовом. Хорошая покраска, легкая продувка и необычное блюдце, где не тухнут угли, а пепел вам не мозолит глаза.  Внутри нержавейка, а на конце шахты есть диффузор, что делает тягу практически бесшумной. Для любителей покурить на 2 шланга есть возможность установки второго комплекта. Так же кальян является монолитным, так что не пытайтесь его разбирать.

                    <p>Покупатели и владельцы данного кальяна открыто говорят - кальян мощный, стильный. Тяга у него между Мамуном и кальяном Otivana - это золотая середина . Цена продукта от CWP не маленькая, но производители заявляют, что прослужит он долго, за что можно заплатить, если вы цените каждую мелочь в кальяне.

                    <p>Просто покурите на этом кальяне, чтобы понять ваш это кальян или нет. Все станет ясно после первой тяги.
                </div>
            </div>
            <div id="haracterist" class="container tab-pane fade"><br>
                <div class="tabs_wrap">
                    <div class="row">
                        <div class="col-md-6">
                            <table class="det_table_haract">
                                <tr>
                                    <td class="table_haract_name">
                                        Высота
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        35 см
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table_haract_name">
                                        Комплектация
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        шахта, блюдце, колба
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table_haract_name">
                                        Материал
                                        колбы
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        Стекло
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table_haract_name">
                                        Материал
                                        шахты
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        Анодированный алюминий,
                                        Нержавейка
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table_haract_name">
                                        Тип соединения
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        Кликовое
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table_haract_name">
                                        Цвет колбы
                                    </td>
                                    <td  class="table_haract_fact text_inf">
                                        Прозрачный
                                    </td>
                                </tr>
                            </table>
                        </div>
                    </div>

                </div>
            </div>
            <div id="comment" class="container tab-pane fade"><br>
                <div class="tabs_wrap">
                    <div class="comment_list_wrap">
                        <div class="row">
                            <div class="col-lg-8">
                                <div class="row ">
                                   <div class="col-3">
                                       <div class="comment_img_wrap">
                                           <img src="image/otz1.png" alt="otz">
                                       </div>
                                   </div>
                                   <div class="col-9">
                                       <div class="comment_text_wrap">
                                           <h4>Денис Привальнев</h4>
                                           <p>
                                               Хороший кальян. Продавец предложил несколько вариантов табака, обсудили все нюансы, также прикупил новую чашу "Облако". Быстро привезли заказ. У ребят доставка кстати от 5000 руб. - Бесплатно.
                                           </p>
                                       </div>
                                   </div>
                               </div>
                                <div class="row">
                                    <div class="col-3">
                                        <div class="comment_img_wrap">
                                            <img src="image/otz1.png" alt="otz">
                                        </div>
                                    </div>
                                    <div class="col-9">
                                        <div class="comment_text_wrap">
                                            <h4>Денис Привальнев</h4>
                                            <p>
                                                Хороший кальян. Продавец предложил несколько вариантов табака, обсудили все нюансы, также прикупил новую чашу "Облако". Быстро привезли заказ. У ребят доставка кстати от 5000 руб. - Бесплатно.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="comment_add_wrap">
                        <div class="row">
                            <div class="col-lg-4">
                                <h4>Оставить свой отзыв</h4>
                                <form action="/action_reg.php" class="main_form">
                                    <div class="form-group">
                                        <input type="text" class="form-control"  placeholder="Имя" name="name" required>
                                    </div>
                                    <div class="form-group">
                                        <input type="email" class="form-control" placeholder="E-mail" name="mail" required>
                                    </div>
                                    <div class="form-group">
                                        <textarea class="form-control" rows="5" maxlength="1000" placeholder="Введите сообщение. Не более 10000 символов"></textarea>
                                    </div>
                                    <button type="submit" class="btn_main">Отправить</button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="sect_wrap">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h2 class="title">С этим товаром покупают</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="slider_product">
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr1.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr2.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr3.png" alt="prod">
                                    <ul class="prod_label_inf">
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <a href="/product/detal.php">
                                        <img src="image/prod/pr4.png" alt="prod">
                                        <ul class="prod_label_inf">
                                            <li>
                                                <img src="image/svg/new.svg" alt="new">
                                            </li>
                                        </ul>
                                    </a>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr5.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr6.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr7.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="sect_wrap">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h2 class="title">Вы недавно смотрели</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="slider_product">
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr1.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr2.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr3.png" alt="prod">
                                    <ul class="prod_label_inf">
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <a href="/product/detal.php">
                                        <img src="image/prod/pr4.png" alt="prod">
                                        <ul class="prod_label_inf">
                                            <li>
                                                <img src="image/svg/new.svg" alt="new">
                                            </li>
                                        </ul>
                                    </a>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr5.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr6.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider_product_item">
                        <div class="product_wrap">
                            <div class="prod_img_wrap">
                                <a href="/product/detal.php">
                                    <img src="image/prod/pr7.png" alt="prod">
                                    <ul class="prod_label_inf">
                                        <li>
                                            <img src="image/svg/new.svg" alt="new">
                                        </li>
                                    </ul>
                                </a>
                            </div>
                            <div class="prod_inf_wrap">
                                <div class="prod_name_wrap">
                                    <p class="text_inf">
                                        Кальян Alpha Hookah X Black
                                        atte (Черный мат)
                                    </p>
                                </div>
                                <div class="prod_prise_wrap">
                                    <div class="price_wrap">
                                        <div class="prise_new_wrap">
                                            <span class="price">2500р</span>
                                        </div>
                                        <div class="prise_old_wrap">
                                            <span class="old_prise text_inf">3500р</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="desk_fav_btn_wrap">
                                    <ul class="shop_icon_list">
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="сравнить">
                                                    <span class="shop_icon_svg">
                                                        <img src="image/svg/sr.svg" alt="Сравнить">
                                                    </span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="shop_icon_link " data-toggle="tooltip" data-placement="top" title="избранное">
                                                    <span class="shop_icon_svg heart">
                                                        <img src="image/svg/iz.svg" alt="избранное">
                                                    </span>
                                            </a>
                                        </li>
                                        <li class="cart_for_mob">
                                            <a href="#" class="shop_icon_link" data-toggle="tooltip" data-placement="top" title="в корзину">
                                                <span class="shop_icon_svg">
                                                    <img src="image/svg/cart_light.svg" alt="Корзина">
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="prod_btn_wrap">
                                    <button class="btn_main">
                                        В корзину
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<?php include_once __DIR__."/footer.php";?>
